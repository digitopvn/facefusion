...
def listen() -> None:
    JOB_RUNNER_JOB_ACTION_DROPDOWN.change(
        update_job_action,
        inputs=JOB_RUNNER_JOB_ACTION_DROPDOWN,
        outputs=JOB_RUNNER_JOB_ID_DROPDOWN,
    )
    JOB_RUNNER_START_BUTTON.click(
        start, outputs=[JOB_RUNNER_START_BUTTON, JOB_RUNNER_STOP_BUTTON]
    )
    JOB_RUNNER_START_BUTTON.click(
        run,
        inputs=[JOB_RUNNER_JOB_ACTION_DROPDOWN, JOB_RUNNER_JOB_ID_DROPDOWN],
        outputs=[
            JOB_RUNNER_START_BUTTON,
            JOB_RUNNER_STOP_BUTTON,
            JOB_RUNNER_JOB_ID_DROPDOWN,
        ],
    )
    JOB_RUNNER_STOP_BUTTON.click(
        stop, outputs=[JOB_RUNNER_START_BUTTON, JOB_RUNNER_STOP_BUTTON]
    )

    ui_workflow_dropdown = get_ui_component("ui_workflow_dropdown")
    if ui_workflow_dropdown:
        ui_workflow_dropdown.change(
            remote_update,
            inputs=ui_workflow_dropdown,
            outputs=[
                JOB_RUNNER_WRAPPER,
                JOB_RUNNER_JOB_ACTION_DROPDOWN,
                JOB_RUNNER_JOB_ID_DROPDOWN,
            ],
        )
...
) -> Tuple[gradio.Row, gradio.Dropdown, gradio.Dropdown]:
    is_job_runner = ui_workflow == "job_runner"
    queued_job_ids = job_manager.find_job_ids("queued") or ["none"]

    return (
        gradio.Row(visible=is_job_runner),
        gradio.Dropdown(
            value=get_first(uis_choices.job_runner_actions),
            choices=uis_choices.job_runner_actions,
        ),
        gradio.Dropdown(value=get_last(queued_job_ids), choices=queued_job_ids),
    )
...
def run(
    job_action: JobRunnerAction, job_id: str
) -> Tuple[gradio.Button, gradio.Button, gradio.Dropdown]:
    ...
    if job_action == "job-run":
        ...
        if job_id and job_runner.run_job(job_id, process_step):
            logger.info(
                wording.get("processing_job_succeed").format(job_id=job_id), __name__
            )
        else:
            logger.info(
                wording.get("processing_job_failed").format(job_id=job_id), __name__
            )
        ...
        return (
            ...
            gradio.Dropdown(value=get_last(updated_job_ids), choices=updated_job_ids),
        )
...
def update_job_action(job_action: JobRunnerAction) -> gradio.Dropdown:
    if job_action == "job-run":
        ...
        return gradio.Dropdown(
            value=get_last(updated_job_ids), choices=updated_job_ids, visible=True
        )
    if job_action == "job-retry":
        updated_job_ids = job_manager.find_job_ids("failed") or ["none"]
