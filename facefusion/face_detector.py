...
from facefusion.face_helper import (
    create_rotated_matrix_and_size,
...
def collect_model_downloads() -> Tuple[DownloadSet, DownloadSet]:
    ...
    if state_manager.get_item("face_detector_model") in ["many", "retinaface"]:
        model_hashes["retinaface"] = (
            MODEL_SET.get("retinaface").get("hashes").get("retinaface")
        )
        model_sources["retinaface"] = (
            MODEL_SET.get("retinaface").get("sources").get("retinaface")
        )
    ...
    if state_manager.get_item("face_detector_model") in ["many", "yoloface"]:
        model_hashes["yoloface"] = (
            MODEL_SET.get("yoloface").get("hashes").get("yoloface")
        )
        model_sources["yoloface"] = (
            MODEL_SET.get("yoloface").get("sources").get("yoloface")
        )
...
def pre_check() -> bool:
    ...
    return conditional_download_hashes(
        download_directory_path, model_hashes
    ) and conditional_download_sources(download_directory_path, model_sources)
...
def detect_faces(
    ...
    print(211)
    if state_manager.get_item("face_detector_model") in ["many", "retinaface"]:
        bounding_boxes, face_scores, face_landmarks_5 = detect_with_retinaface(
            vision_frame, state_manager.get_item("face_detector_size")
        )
    ...
    if state_manager.get_item("face_detector_model") in ["many", "scrfd"]:
        bounding_boxes, face_scores, face_landmarks_5 = detect_with_scrfd(
            vision_frame, state_manager.get_item("face_detector_size")
        )
        all_bounding_boxes.extend(bounding_boxes)
        all_face_scores.extend(face_scores)
    ...
    if state_manager.get_item("face_detector_model") in ["many", "yoloface"]:
        print(2130)
        bounding_boxes, face_scores, face_landmarks_5 = detect_with_yoloface(
            vision_frame, state_manager.get_item("face_detector_size")
        )
        print(2131)
        all_bounding_boxes.extend(bounding_boxes)
    ...
    all_bounding_boxes = [
        normalize_bounding_box(all_bounding_box)
        for all_bounding_box in all_bounding_boxes
    ]
    return all_bounding_boxes, all_face_scores, all_face_landmarks_5


def detect_rotated_faces(
    vision_frame: VisionFrame, angle: Angle
) -> Tuple[List[BoundingBox], List[Score], List[FaceLandmark5]]:
    rotated_matrix, rotated_size = create_rotated_matrix_and_size(
        angle, vision_frame.shape[:2][::-1]
    )
    ...
    bounding_boxes = [
        transform_bounding_box(bounding_box, rotated_inverse_matrix)
        for bounding_box in bounding_boxes
    ]
    face_landmarks_5 = [
        transform_points(face_landmark_5, rotated_inverse_matrix)
...
def detect_with_retinaface(
    vision_frame: VisionFrame, face_detector_size: str
) -> Tuple[List[BoundingBox], List[Score], List[FaceLandmark5]]:
    bounding_boxes = []
    face_scores = []
    face_landmarks_5 = []
    ...
    for index, feature_stride in enumerate(feature_strides):
        keep_indices = numpy.where(
            detection[index] >= state_manager.get_item("face_detector_score")
        )[0]
...
def detect_with_yoloface(
    ...
    bounding_box_raw, score_raw, face_landmark_5_raw = numpy.split(
        detection, [4, 5], axis=1
    )
    print(2306)
    keep_indices = numpy.where(
        score_raw > state_manager.get_item("face_detector_score")
    )[0]
    print(231)
...
def prepare_detect_frame(
    temp_vision_frame: VisionFrame, face_detector_size: str
) -> VisionFrame:
    print(2311)
    face_detector_width, face_detector_height = unpack_resolution(face_detector_size)
    print(2312)
    ...
    print(2313)
    print(temp_vision_frame.shape[0])
    print(temp_vision_frame.shape[1])
