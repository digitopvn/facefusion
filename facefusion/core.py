...
def pre_check() -> bool:
    if sys.version_info < (3, 9):
        logger.error(
            wording.get("python_not_supported").format(version="3.9"), __name__
        )
        return False
        ...
        if hasattr(module, "MODEL_SET"):
            for model in module.MODEL_SET.values():
                ...
                if model_hashes and model_sources:
                    ...
                    ) or not conditional_download_sources(
...
def process_image(start_time: float) -> ErrorCode:
    ...
    for processor_module in get_processors_modules(
        ...
        processor_module.process_image(
            state_manager.get_item("source_paths"), temp_file_path, temp_file_path
        )
    ...
    if is_image(state_manager.get_item("output_path")):
        ...
        logger.info(
            wording.get("processing_image_succeed").format(seconds=seconds), __name__
        )
...
def process_video(start_time: float) -> ErrorCode:
    ...
    if extract_frames(
        state_manager.get_item("target_path"), temp_video_resolution, temp_video_fps
    ):
    ...
    if is_video(state_manager.get_item("output_path")):
        ...
        logger.info(
            wording.get("processing_video_succeed").format(seconds=seconds), __name__
        )
        conditional_log_statistics()
    else:
